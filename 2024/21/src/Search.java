...
public class Search
{
  ...
  public static List<State> searchM(List<State> starts)
  {
    ...
    LinkedList<State> sols = new LinkedList<>();
    ...
    while(!queue.isEmpty())
    {
      ...
      if (sols.size() > 0)
      if (s.getCost() + s.getEstimate() > best_sol + Prob.allow_skew)
      {
        return sols;
      }
      ...
      if (s.isTerm())
      {
        ...
        {
          best_sol = Math.min(best_sol, s.getCost());
        }
        sols.add(s);

        System.out.println("P Search solution: " + s);
        //return s;
      }
